...
4. ✅ **Git 提交**: 在 feature 分支上执行 `git commit` 并附带完整的特性描述
5. ✅ **合并分支**: 切换到 main，合并 feature 分支
6. ✅ **Commit 格式**:
   ```
   <type>: <完整的特性描述>
...
**合并后的强制条件**:
- ⛔ 禁止使用 `git push --force` 到 main
- ⛔ 合并前必须确保 main 是最新
- ⛔ 合并后必须删除已合并的 feature 分支
...
1. **开发必须在新分支进行**:
   - ⛔ **禁止在 main 分支直接开发或提交**
   - 每个功能/任务创建新的 feature 分支
   - 分支命名格式: `feature/<功能描述>` 或 `fix/<问题描述>`
...
**分支定义**:
- `main` - 主分支，稳定代码，只接受 merge
- `feature/*` - 功能分支，每个新功能一个分支
- `fix/*` - 修复分支，每个 bug 修复一个分支
...
- [ ] **功能完成后：合并 feature 分支到 main 并删除开发分支**
...
1. **创建开发分支** - 开发前必须从 main 创建新的 feature 分支
2. **激活 venv 环境** - 执行任何 Python 操作前
3. **会话恢复检查** - 新会话开始时，先读取记忆和会话状态
4. **读取最新计划** - 确认当前执行的计划文件（最新版本）
5. **严格执行计划** - 按计划文件中的步骤执行，不偏离
6. **TDD 开发** - 先写测试，后写代码
7. **验证完成度** - 确认功能完善、测试全通过
8. **代码审查** - 使用 code-reviewer 代理
9. **Git 提交** - 在 feature 分支上提交代码
10. **合并分支** - 功能完成后合并 feature 分支到 main
11. **清理分支** - 合并后删除已合并的 feature 分支
12. **更新记忆** - 会话结束时更新会话状态和项目记忆
13. **更新计划** - 迭代完成后创建新版本计划文件
...
# 合并 feature 分支到 main
git merge --no-ff feature/<功能名> -m "Merge branch 'feature/<功能名>'"
